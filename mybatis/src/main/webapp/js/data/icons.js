        ...
        "columns" : [
            ...
            {"orderable" : false,
                "render" :  function (data, type, row) {
                    return '<img width="20px" src="' + row.path + '"/>';
                }
            },
            {"data" : "name"},
            {"data" : "path"},
            {"orderable" : false,
                "render" :  function (data, type, row) {
                    ...
                    current_page_row_buttons.forEach(function (value) {
                        buttons = buttons + value.replace(/{{iconId}}/g, iconId) + '&nbsp;';
                    });
                    return buttons;
                }
            }
...
function deleteRow(obj) {
    ...
    $.ajax({
        type : 'post',
        url : '/deleteIconById?iconId=' + iconId,
        success : function (resp) {
            if(resp.code === 200) {
                showMsg("删除成功");
                table.draw();
